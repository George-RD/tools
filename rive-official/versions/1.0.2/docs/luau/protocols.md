...
| `Tests` | `setup(test: Tester)` | unit tests, run headless | — |
...
Reach for the format first. `Feather` softens a fill or stroke, `blendModeValue`
...
So the pattern is one scripted piece among markup, not a scripted scene:
...
<ScriptAsset file="ripple.luau" name="ripple" id="0:80"/>
```

**Text is the exception, and it is a hard one.** There is no text API, so a
script can never draw a string. An effect *on* type has to be built around it --
a `ScriptedDrawable` behind or in front of a real `Text`, or a path effect on a
shape beside it -- never by scripting the text itself.
...
`io`, no `coroutine`, and no `debug`** — a script cannot touch the filesystem or
yield.
...
`--verify` is mutually exclusive with `--once`, `--publish` and `--test`, and
`--rev=` cannot combine with it. Of those, only `--publish` needs a session —
as does `--rev=`, which exports the editor file.
...
to lay it out against without one. The editor repairs a missing style on nested
layouts but deliberately skips artboards, so an artboard without one is never
fixed up: the CLI renders the scene, and the same file opened in the editor does
not. See [../layout.md](../layout.md).
...
return function(): Node<Oval>
    return { path = Path.new(), update = update, draw = draw }
end
```

Attach it with a `ScriptedDrawable`, which is a `Drawable`: it carries `x`/`y`,
a transform and its own `blendModeValue`, so it sits among ordinary siblings
and obeys the same paint order.
...
`ListenerViewModelChange`. `perform` returns **nothing** — it is called for its
effect.

```luau
type Act = {}

local function perform(self: Act, pointerEvent: PointerEvent)
    print('clicked')
end
...
`performAction` is the other hook, taking a `ListenerContext` instead of the
pointer event. Both are optional, so a script with neither compiles and does
nothing.
