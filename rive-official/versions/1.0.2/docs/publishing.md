...
**`--publish` needs a session**, because it signs through the API, and so does
`--rev`, which exports the editor file a designer opens. The live window,
`--once`, `--verify`, `--test` and `--screenshot` are entirely local and run
signed out.

That matters offline as well as signed out: `requireLoggedIn` is a live
`whoami`, so a gated mode fails on a dropped connection even with valid stored
credentials.

So without an account you can author, compile, watch, screenshot, run tests
and produce a `.riv`. What needs a session is handing the work on: `--publish`
to ship it, `--rev` to pass it to the editor.
...
`--format=json` on `--once`, `--verify`, `--publish` or `--test` prints one
JSON envelope on stdout — `{"success", "command", "data", "errors",
"warnings"}` — with the problem list (script, line, message) or the test
results (counts plus each failure's group path) under `data`. Logs stay on
stderr, so stdout is only the envelope.
