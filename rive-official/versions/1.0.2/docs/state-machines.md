...
> [data.md](data.md).
>
> **One exception, and it matters: a component's own internal behaviour.** A
> nested artboard inherits its parent's data context rather than binding its
> own `viewModelInstanceId`, so a view-model-driven machine works perfectly
> while the component artboard is the root and does nothing at all once the
> component is placed. The states are right, the machine advances, the
> listeners fire — the transition condition just never reads true. Nothing
> reports it. For a machine that lives inside something nestable, drive it with
> an input, which belongs to the machine and travels with it.
...
- **`StateMachine`** — named by the artboard's `defaultStateMachineId` so it
  plays on load. Without that link the machine exists and never runs — and the
  artboard receives no data and no pointer input either, however complete the
  rest of the file is
...
states in [easing.md](easing.md#blending-from-a-view-model).
...
`greaterThan`, `greaterThanOrEqual`. Boolean conditions only use the first two
and have no value of their own — see
[format.md](format.md#inputs-and-conditions), the one part of this that the
type's shape does not reveal.
...
Listeners turn clicks, hovers and drags into input changes. They live on the
`StateMachine`, not in a layer, and are covered in
[format.md](format.md#listeners).

The pattern is always the same: a listener sets an input, a condition reads it.
Listeners do not change states directly.

**Every listener needs a `targetId`.** It is the object whose hit area the
listener watches, and a listener without one resolves to nothing and never
fires — no error, nothing in `problems`, the machine simply sits there. Pointer
...
Audio needs the reporting more than urls do: a sound that played and one that
did not are the same silence in a screenshot, and every way
`AudioEvent::play()` gives up is a silent return.

**`triggered` is not `played`.** The player is handed the events *before* the
runtime plays them, so it can only report the checks it can make itself. The
three failures are exact, but past them `play()` can still find no audio engine,
or the engine can decline the sound, and neither is visible from here.
...
That reports a machine no artboard plays — but **not** a transition pointing at
a state that does not exist, an `AnimationState` naming a missing animation, or
a condition naming a missing input. Those ids are never resolved, so all three
inspect perfectly clean and the machine simply sits still. Confirm them
yourself:
