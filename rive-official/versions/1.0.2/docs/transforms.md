...
This is rare — five uses across the whole reference corpus — and reordering the
tree is almost always simpler. Reach for it only when the order has to change
over time.
...
[drawing.md](drawing.md#effects-and-clipping). It is worth knowing that clip paths follow
the fill's post-effect path, so a clipped shape with a `Feather` or trimmed path
clips to the visible result rather than the underlying geometry.
