...
unbound.** A vertex whose only slot is `0` keeps its bind position while
everything around it deforms, which looks exactly like bones not working at all.
This is also why the property's default is `1` — "the first tendon", not "the
second".

Tendons 0 and 1 at half influence each is therefore:

```
indices = 1 | (2 << 8) = 513
values  = 128 | (128 << 8) = 32896
```

```xml
<Weight values="32896" indices="513"/>
```
...
**Two ways a mesh builds clean and then misbehaves**, neither reported by
`--verify` or `problems`, and they fail very differently:
...
- **The `Mesh` is not a child of an `Image`.** Nothing else can host one, and a
  ...
  undeformed, and moving the bones does nothing to it.
- **`triangleIndexBytes` is missing, or an index is `>=` the vertex count.**
  ...
  Omitting the attribute entirely still passes `--verify` with `problems` empty.

`inspect` is no help here — `Bytes` properties are omitted from its output the
way `List<Id>` ones are, so `triangleIndexBytes` never appears whether you wrote
it or not. Take a `--screenshot` instead: a blank capture where the artboard
should be is the second failure, and an image that refuses to bend is the first.

### Take it from a `.rev`

Skinning is the one part of the format that is genuinely painful to author by
hand — the packed weights are unreadable, the bind pose has to match the bones'
actual transforms, and a mesh adds an encoded index buffer whose contents
nothing validates until the file is loaded. If the geometry comes from a
designer, take the skinned path or mesh from a decompiled `.rev` rather than
writing it.
