...
A screenshot captures **frame 0**, and a well-built loop is at its resting value
there — that is what makes the seam invisible. So a correct animation and a
completely broken one produce the same first frame, and a screenshot alone tells
you nothing.

`--advance=<N>` steps the file at 60fps before capturing. Take three:

```bash
rive <dir> --screenshot=f0.png                  # rest
...
once, weighted by a number input — a walk/run cycle driven by speed, or a face
that follows a cursor.

`BlendState1DInput` blends along a single axis:

```xml
<StateMachineNumber name="speed" id="0:60"/>
...
<BlendState1DInput inputId="0:60" id="0:70">
...
Each `BlendAnimation1D` pins an animation to a point on the input's range with
`value`. At `speed = 0` the first plays alone; at `75` the runtime mixes the
second and third.
...
### Blending from a view model

`BlendState1DInput` is named for what drives it, and state machine inputs are
deprecated. **`BlendState1DViewModel` is the current form**: same blend state,
same `BlendAnimation1D` children, but the axis comes from a view model property
instead of a `StateMachineNumber`.

It has no `inputId`. Nest a `BindableProperty*` and that *is* the wiring:
...
The same swap works per-animation in a `BlendStateDirect`. `BlendAnimationDirect`
