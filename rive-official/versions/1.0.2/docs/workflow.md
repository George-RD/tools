...
runs with `File::deterministicMode` on, so scroll physics derives velocity from
...
through the same `gamepadConnected` a real one does, and `evi::GamepadInfo`
carries no flag to report anything but the standard mapping.
