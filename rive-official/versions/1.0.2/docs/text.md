...
Three links to get right, and each fails silently on its own:
...
None of these are errors, and — unlike bind paths — **none of them appear in
`problems` either.** A `styleId` pointing at nothing, or a `FontAsset` naming a
file that is not on disk, both inspect completely clean. Checking them means
cross-referencing the ids in the tree yourself:

```bash
# every run's styleId, and every style's id -- they should line up
rive inspect . --json | jq -c '[..|objects|select((.type//"")=="TextValueRun")|.styleId],
                               [..|objects|select((.type//"")=="TextStylePaint")|.id]'
```

Text that renders as nothing, with a clean build and an empty `problems`, is
...
`familyName` and `styleName` on the style are metadata describing the face; the
asset reference is what actually loads it.
...
`--screenshot` or the watch preview renders the text as nothing at all. That is
**identical** to what a misspelled `file` looks like
([gotchas.md](gotchas.md#a-missing-asset-file-reports-nothing-at-all)), so the
two are easy to confuse — check whether the attribute is deliberately absent
before hunting for a broken path.
...
Neither is used anywhere in the reference corpus, so treat the details as
derived from the schema rather than from working files — build a small one and
`rive inspect` it before relying on the shape.
