...
Prints the resolved scene tree, plus a `problems`
list of everything wrong with it.
...
{"severity": "warning", "kind": "no-default-state-machine", "line": 3,
 "message": "artboard \"A\" declares animations but no default state machine"}
```
...
| `no-default-state-machine` | an artboard with animations that plays none |
...
- **An id that is set but points at nothing.** `stateToId`, `animationId`,
  `inputId`, `styleId`, `targetId`, `viewModelPropertyId`, `converterId` — none
  are resolved. Only bind *paths* are. The rule is that `problems` checks a
  required reference is *present*, not that it *resolves*.
- **An asset file that is not on disk.** Nothing reports it.
...
The single most common failure mode is a file that builds, runs, renders
something, and is quietly not what was asked for. Guard against it:

1. `rive <dir> --verify` — did it compile?
2. `rive inspect <dir> --json` — is `problems` empty?
3. Query the tree for what you intended to build, rather than eyeballing it.

Do not save this until the end. Build a screen in passes — a wireframe first,
then detail — and run these after each one, so a structural mistake surfaces
while it is still cheap to fix. See [workflow.md](workflow.md).

For example, after keying a rotation:

```bash
rive inspect . --json | jq '[..|objects|select(.type=="KeyedProperty")|.propertyKey]'
```

If that comes back empty, nothing is animated, however well the build went.
